---
title: Contributing to Ladybird
description: Guidelines and policies for contributing to the Ladybird Browser project
---
...
- Follow the [detailed issue-reporting guidelines](https://github.com/LadybirdBrowser/ladybird/blob/master/ISSUES.md).
...
<Note>
This also applies to debug logging and other internal strings, as they may be exposed to users in the future.
</Note>
...
- Conform to the project coding style found in [CodingStyle.md](/development/coding-style). Use `clang-format` to automatically format C++ files.
...
<Accordion title="Is there a policy for branches/PRs that haven't been touched in X days? Should they be closed?">
...
<Steps>
  <Step title="Install pre-commit hook">
    Runs Meta/lint-ci.sh and Meta/lint-ports.py to ensure changes to the code will pass linting:
  ...
  <Step title="Install post-commit hook">
    Lints the commit message to ensure it will pass the commit linting:
...
<Note>
The `upstream` remote in this command should be replaced with whatever you've named the LadybirdBrowser/ladybird.git remote in your local clone. Use `git remote -v` to find that name.
</Note>
